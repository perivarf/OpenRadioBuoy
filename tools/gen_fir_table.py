...
/*
  ...
  Windowed-sinc lowpass (Hamming, normalised so sum(h) = 1), from the same function
  postprocess.py uses offline: fir.firwin_lowpass. Sum = 1 gives exactly unity gain at
  DC, so any residual accel bias is not scaled.

  ONE TABLE PER DECIMATION FACTOR, not per rate pair. The coefficients depend only on
  fc/fs, and since both stages use fc = fs_out/2 that is 1/(2*D) - so 960->100 and
  480->50 are the SAME table, bit for bit. That is why the rates can change in
  analysis_config.h without regenerating anything here.

  The name suffix is 10x the decimation factor, since D can be 9.6:
  kFirTapsD96 = D 9.6, kFirTapsD100 = D 10.0.

  This file knows no rates and deliberately does NOT include the config headers -
  analysis_config.h includes this one and does the lookup. The other way round is
  circular.

  The tables live in flash ({len(tables)} x {args.ntap * 4} B in source); the linker drops the
  unused ones (-fdata-sections + --gc-sections), so only the two selected cost anything.
  Only FirDecimator's delay lines cost RAM.
*/

// The tap count is baked into the tables. Changing it without regenerating makes the
// filter quietly wrong - hence a fence rather than a comment.
